...
把原来的分散搜索和测试式结果页重构为可日常使用的统一校招网申表格。

### Added

- 新增 `/api/opportunities`，统一返回具体岗位和仅有公告依据的招聘项目。
- 新增 58 个条目的官方招聘来源注册表，覆盖央国企、科技公司、银行、公共就业和招聘平台。
- 新增无 API 密钥的 Google News RSS 招聘公告发现，支持当前届别的秋招、校招、提前批和启动公告组合查询。
- 新增公司、届别和招聘类型级别的跨媒体公告去重。
- 新增来源等级、企业类型、行业、新鲜度、过期状态和招聘批次筛选。
- 新增表格列排序、快捷筛选、每日自动刷新、键盘可操作的左右分隔条和手机端结果布局。
- 新增统一机会、过期过滤、新闻解析、低质结果拦截和公告去重测试。

### Changed

- 前端改为左侧集中设置、右侧网申表格；搜索入口合并为一次“搜索并刷新”。
- 搜索先返回本地结果，联网刷新完成后自动合并，不再要求用户手动导入。
- 无法抽出明确岗位的有效招聘公告进入“招聘项目”，不再伪造成岗位。
- 已过截止日期的数据默认隐藏；缺少截止日期和届别依据的旧记录标为“待确认”。
- Google、Bing 和搜狗微信保留为复合搜索来源，搜狗微信继续负责公众号专项发现。
- 启动时清理旧版示例公司、岗位和公众号文章，不再生成演示结果。

### Verified

- 34 个单元测试通过。
- `apps/web/app.js` 通过 Node 语法检查。
- 使用真实“秋招”和“国家能源集团”查询验证了公告项目与具体岗位的混合结果。
- 桌面和手机视口完成浏览器交互与布局检查。
