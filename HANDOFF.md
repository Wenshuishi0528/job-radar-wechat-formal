...
2. `source_registry.py` 提供 58 个已知正式来源；泛词会遍历来源入口，公司词只匹配对应来源。
...
7. `repository.py` 负责入库、状态维护、统一筛选、投递跟踪、分类校正和过期处理。
8. `opportunity_matcher.py` 在本机按简历技能、岗位方向、城市、学历和来源评分，不依赖外部模型。
...
- `services/api/app/opportunity_matcher.py`：无 API 的本地简历机会评分。
- `apps/web/app.js`：统一搜索、筛选、匹配、投递跟踪、分页、CSV 和自动刷新。
...
7. 已提供投递进度和 CSV；尚未提供系统通知、日历订阅、多个保存画像和真正的大模型语义匹配。
8. 泛词会给出大量可信官网入口，但只有具备稳定解析器或公开静态结构的来源才能下钻成具体岗位。
...
2. 增加系统截止提醒和日历订阅。
3. 增加 Excel 导出。
4. 增加 PDF、图片 OCR 与复杂职位表解析，并优先扩充具体岗位级解析器。
