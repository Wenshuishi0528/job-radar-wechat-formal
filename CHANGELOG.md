...
把网申表格补成可日常管理的个人求职工作台，并扩大泛词搜索的可信覆盖。

### Added

- 新增本地简历文本匹配，可按技能、岗位方向、城市、学历和来源对全部机会评分排序。
- 新增投递跟踪：收藏、准备中、已投递、测评、面试、Offer、未通过、放弃、备注和下一步日期。
- 新增岗位方向、专业和投递状态筛选，以及 25/50/100/200 分页和 CSV 导出。
- “秋招/校招/实习”等泛词自动加入 58 个企业官网、政府招聘页和招聘平台入口。
- 搜狗微信解析器适配新版结果页的标题、摘要、公众号名、时间和加密跳转链接。

### Changed

- Google News 结果会继续尝试普通 Google 网页搜索补充，不再命中新闻后提前结束。
- 手机端默认折叠高级筛选，让搜索按钮和结果入口进入首屏；桌面仍保留完整左侧设置栏。
- 国家能源岗位页读取列表中的截止日期，并自动校正既有岗位的分类和过期状态。
- Bing 返回无关公共搜索页时明确报告通道降级，不计入有效候选。

### Fixed

- 修复教师、政策研究、矿井通风和工程技术岗位被“研究/产品”等宽泛词误分类的问题。
- 修复简历匹配结果又被更新时间重排的问题。
- 修复字符串 `"false"` 被当成真值，导致已截止岗位进入默认简历匹配的问题。
- 修复桌面表格操作列被横向裁切以及手机首屏被高级筛选占满的问题。

### Verified

- 45 个单元测试通过，Node 语法检查和 `git diff --check` 通过。
- 真实“秋招”复合刷新整理 58 个正式入口、33 条 Google 结果和 30 条搜狗结果；Bing 异常被独立报告。
- 投递记录增删、投递视图、简历匹配排序、分页、桌面 1440px 和手机 390px 布局均完成浏览器验收，控制台无警告或错误。
